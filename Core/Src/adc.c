...
******************************************************************************
*
* note the STM32C011F4 in UFQFPN comes only in -40C to 85C industrial variant
* it has 16k flash 6k ram.
* a pin compatible is STM32C011F6 with 32k flash 6k ram. also industrial.
* there is a CAN variant STM32C092EC with 256k flash 30k ram.
* has industrial U3TR suffix, and extended part U6TR suffix.
*
* the temperature drift of the voltage reference is 50ppm/K typical 100ppm/K max
* total unadjusted error after calibration, of the ADC is
*        3 counts typical 6.5 counts max, 0.07% to 0.15%
* the tolerance on the 1.2V internal reference VREFINT is 1% typical 3% max at 30C
* the tolerance of the 3.3V regulator -40C to 150C is 1.5%
*
* conclusion:
* min accuracy with ADC and Vdd calibration -25C to 85C is 0.7%
* min accuracy with ADC calibration -40C to 85C is 1.7%
* min accuracy without calibration is not guaranteed
* the error contributed by the ADC after calibration seems to be very low
*        so it may not be necessary to calibrate the ADC more than once
*        even in the presence of temperature change.
*
******************************************************************************
*
* temp sensor startup time is 120us max
* required sample time is 5us
*
* vrefint startup time is 12us max
* required sample time is 4us
*
* ADC clock is 24MHz. number of clock ticks to complete one conversion
* cycle is 160x2 + 1x5 + 13x7 = 320+5+91 = 416ticks = 17.4us.
* therefore max detectable frequency is 29kHz.
*
******************************************************************************
*/
